...
import cn.wnhyang.coolguard.decision.constant.DisposalConstant;
...
import cn.wnhyang.coolguard.decision.vo.result.PolicyResult;
import cn.wnhyang.coolguard.decision.vo.result.PolicySetResult;
import cn.wnhyang.coolguard.decision.vo.result.RuleResult;
import lombok.Data;
...
public class PolicyContext {
    ...
    public PolicySetResult convert() {
        ...
        for (Map.Entry<String, PolicyCtx> entry : policyMap.entrySet()) {
            ...
            // 模拟规则
            List<RuleCtx> mockRuleList = hitMockRuleListMap.get(policy.getCode());
            if (CollUtil.isNotEmpty(mockRuleList)) {
                mockRuleList.forEach(rule -> {
                    RuleResult ruleResult = new RuleResult(rule.getName(), rule.getCode(), rule.getExpress());
